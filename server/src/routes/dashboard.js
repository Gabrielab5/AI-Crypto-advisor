...
//  CoinGecko: 60s  |  CryptoPanic: 5min  |  AI insight: 10min
const TTL = {
  coingecko:   60 * 1000,
  cryptopanic: 5  * 60 * 1000,
  ai_insight:  10 * 60 * 1000,
};

const _live  = new Map(); // key → { data, at }  — evicted after TTL
const _stale = new Map(); // key → data           — kept forever (last good value)
...
function setCached(key, data) {
  _live.set(key,  { data, at: Date.now() });
  _stale.set(key, data);
}
...
function filterCoins(all, prefs) {
  ...
  const preferred = ids.length > 0 ? all.filter(c => ids.includes(c.id)) : [];
  ...
  return result.slice(0, 8).map(c => ({
    ...
    change_24h: c.price_change_percentage_24h ?? 0, market_cap: c.market_cap,
  }));
}
...
async function fetchMarketNews(prefs) {
  ...
  try {
    const assets = (prefs.interested_assets || [])
      .filter(a => ['BTC','ETH','SOL','BNB','XRP'].includes(a)).join(',');
    const qs  = assets ? `&currencies=${assets}` : '';
    const res = await fetch(
      `https://cryptopanic.com/api/v1/posts/?auth_token=${process.env.CRYPTOPANIC_API_KEY}&public=true${qs}`,
      { signal: AbortSignal.timeout(6000) }
    );
    if (!res.ok) {
      const stale = getStale('cryptopanic');
      return stale ? { data: stale, stale: true } : { data: FALLBACK_NEWS, stale: false };
    }
...
const STATIC_INSIGHT = {
  text: 'The crypto market rewards patience and consistent research over reactive trading. Layer 2 solutions and DeFi protocols are reshaping how value moves on-chain, creating opportunities for long-term holders. Focus on fundamentals, manage your risk, and avoid letting short-term volatility override your strategy.',
  model: 'static',
...
async function callOpenRouter(prompt) {
  ...
  for (const model of models) {
    try {
      ...
      if (!res.ok) {
        console.error(`[OpenRouter] ${model} HTTP ${res.status}:`, JSON.stringify(json));
        continue;
      }
      if (json.error) {
        console.error(`[OpenRouter] ${model} API error:`, JSON.stringify(json.error));
        continue;
      }
      const text = json.choices?.[0]?.message?.content?.trim();
      if (!text) { console.error(`[OpenRouter] ${model} empty response:`, JSON.stringify(json)); continue; }
      return { text, model: json.model ?? model };
    } catch (err) {
      console.error(`[OpenRouter] ${model} fetch error:`, err.message);
    }
  }
  return null;
}
    ...
    const res = await fetch(
      'https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.2',
      {
        ...
        body: JSON.stringify({
          ...
          parameters: { max_new_tokens: 200, return_full_text: false, temperature: 0.75 },
        }),
        signal: AbortSignal.timeout(20000),
      }
    );
    const json = await res.json();
    if (!res.ok) { console.error('[HuggingFace] error:', JSON.stringify(json)); return null; }
    const text = Array.isArray(json) ? json[0]?.generated_text?.trim() : json?.generated_text?.trim();
    if (!text) return null;
    return { text, model: 'huggingface/mistral-7b-instruct-v0.2' };
  } catch (err) {
...
async function fetchAIInsight(prefs) {
  const cached = getCached('ai_insight');
  if (cached) return { data: cached, stale: false };
  ...
  const prompt      = `Give a short 3-sentence crypto market insight for a ${investorType} investor interested in ${assets}. Today is ${today}. Be concise, specific, and actionable. Do not use bullet points or headers.`;
  ...
  if (!result) {
    const stale = getStale('ai_insight');
...
router.get('/', requireAuth, async (req, res) => {
  ...
  res.json({
    coin_prices: extract(pricesResult, []),
